new CustomApexChart({
  ...
  options: () => ({
    chart: { height: 350, type: "candlestick", toolbar: { show: !1 } },
    plotOptions: {
      candlestick: {
        colors: { upward: [ins("success")], downward: [ins("danger")] },
      },
    },
    series: [{ data: seriesData }],
    stroke: { show: !0, colors: [ins("border-color")], width: 1 },
    xaxis: { type: "datetime" },
    yaxis: { labels: { offsetX: -10 } },
    grid: {
      borderColor: [ins("border-color")],
      padding: { top: -15, right: 10, bottom: 0, left: 0 },
    },
  }),
}),
  new CustomApexChart({
    ...
    options: () => ({
      ...
      plotOptions: {
        bar: {
          ...
          colors: {
            ranges: [
              { from: -1e3, to: 0, color: ins("success") },
              { from: 1, to: 1e4, color: ins("info") },
            ],
          },
        },
      },
  ...
  new CustomApexChart({
    ...
    options: () => ({
      ...
      chart: { height: 380, type: "candlestick", toolbar: { show: !1 } },
      grid: {
        borderColor: [ins("border-color")],
        padding: { top: -15, right: 10, bottom: -20, left: 0 },
      },
      annotations: {
        xaxis: [
          {
      ...
      yaxis: { labels: { offsetX: -10 }, tooltip: { enabled: !0 } },
    }),
  }),
  new CustomApexChart({
    ...
    options: () => ({
      ...
      stroke: { width: [3, 1] },
      colors: [ins("secondary"), ins("purple")],
      ...
      tooltip: {
        ...
        custom: function ({
          ...
          return "line" === n.config.series[t].type
            ? e[t][a]
            : "candlestick" === n.config.series[t].type
            ? `
                <div class="apexcharts-tooltip-candlestick px-3 py-2">
                    <div class="pb-2">Open: <span class="value fw-semibold">${n.globals.seriesCandleO[t][a]}</span></div>
                    <div class="pb-2">High: <span class="value fw-semibold">${n.globals.seriesCandleH[t][a]}</span></div>
                    <div class="pb-2">Low: <span class="value fw-semibold">${n.globals.seriesCandleL[t][a]}</span></div>
                    <div>Close: <span class="value fw-semibold">${n.globals.seriesCandleC[t][a]}</span></div>
                </div>
            `
            : "";
        },

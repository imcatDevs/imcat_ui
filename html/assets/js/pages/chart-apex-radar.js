new CustomApexChart({
  ...
  options: () => ({
    ...
    series: [{ name: "Series 1", data: [85, 70, 60, 90, 75, 65] }],
  ...
  new CustomApexChart({
    ...
    options: () => ({
      ...
      series: [{ name: "Activity Level", data: [80, 60, 75, 90, 50, 70, 65] }],
      ...
      markers: { size: 4, colors: [ins("danger")], strokeWidth: 2 },
      tooltip: {
        y: {
...
let multiSeriesRadarChart = new CustomApexChart({
...
function update() {
  function a() {
    var e = [];
    for (let a = 0; a < 6; a++) e.push(Math.floor(100 * Math.random()));
    return e;
  }
  multiSeriesRadarChart.chart.updateSeries([
    { name: "Marketing", data: a() },
    { name: "Sales", data: a() },
    { name: "IT", data: a() },
  ]);
}

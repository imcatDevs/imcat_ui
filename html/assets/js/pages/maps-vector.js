class VectorMap {
  ...
  initVectorMap(i, a = {}) {
    let e = null;
    if ((e = i instanceof Element ? i : document.querySelector(i))) {
      let i = new jsVectorMap({ selector: e, ...a });
  ...
  initWorldMapMarker() {
    this.initVectorMap("#world-map-markers", {
      ...
      labels: { markers: { render: (i) => i.name } },
    });
  }
  initWorldMarkerLine() {
  ...
  initUsMap() {
    this.initVectorMap("#usa-vector-map", {
      map: "us_aea_en",
      regionStyle: { initial: { fill: ins("primary") } },
    });
  }
  initIndiaMap() {
  ...
  initRussiaMap() {
    this.initVectorMap("#russia-vector-map", {
      map: "russia",
      zoomOnScroll: !1,
      regionStyle: { initial: { fill: "#aab9d14d" } },
    });
  }
  initIraqMap() {
